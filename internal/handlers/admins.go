...
func (app *Application) PostAdminSignup(w http.ResponseWriter, r *http.Request) {
	...
	token, err := app.services.Admins.Signup(
		input.FullName,
		input.Email,
		input.Password,
	)
...
func (app *Application) PostAdminLogin(w http.ResponseWriter, r *http.Request) {
	...
	token, err := app.services.Admins.Login(
		input.Email,
		input.Password,
	)
	if err != nil {
...
func (app *Application) PostAdminLogout(w http.ResponseWriter, r *http.Request) {
	...
	err = app.services.Admins.Logout(token)
	if err != nil {
		err = app.writeJSON(w, http.StatusOK, envelope{"message": "logged out successfully"}, nil)
		if err != nil {
			app.serverErrorResponse(w, r, err)
